...
def generate_random_filename():
    ...
    # 组合成文件名    
    filename = f"{current_date}{random_digits}"
    return filename
...
def convert_wav_to_mp3(wav_path, mp3_path):
    ...
    Args:
        wav_path: WAV 文件的路径。
        mp3_path: 输出 MP3 文件的路径。
    """
    try:
        audio = AudioSegment.from_wav(wav_path)
        audio.export(mp3_path, format="mp3")
        print(f"成功将 {wav_path} 转换为 {mp3_path}")
...
def download_and_convert(mp3_url, wav_filename):
    ...
    """
    try:
        response = requests.get(mp3_url,stream=True,timeout=50)
        ...
        try:
            subprocess.run(['ffmpeg','-y','-i', mp3_file, wav_filename], check=True)
            print(f"成功将 {mp3_file} 转换为 {wav_filename}")
        except FileNotFoundError:
            print(f"找不到文件: {mp3_file} 或 ffmpeg")
...
def sft_get():
    ...
    filetype = request.args.get('filetype',"mp3")
    ...
    if streaming == 0:
        ...
        torchaudio.save(f"音频输出/{filename}.wav",audio_data,22050,format="wav")

        shutil.copyfile(f"音频输出/output.srt",f"{ROOT_DIR}/音频输出/{filename}.srt")
        
        convert_wav_to_mp3(f"音频输出/{filename}.wav",f"音频输出/{filename}.mp3")
        
        if filetype == "wav":
            res = {"status":"ok","url":request.host_url+f"file/{filename}.wav"}
        elif filetype == "mp3":
            res = {"status":"ok","url":request.host_url+f"file/{filename}.mp3"}
        elif filetype == "srt":
            res = {"status":"ok","url":request.host_url+f"file/{filename}.srt"}
        
        resp = make_response(json.dumps(res, ensure_ascii=False, indent=2)) # indent 可选，用于格式化输出
